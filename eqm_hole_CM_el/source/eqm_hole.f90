...
integer :: iout,i,j,nlev,jmax,ih_lev,ip_lev,ii,ipozz,ns
...
double precision, dimension(:,:), allocatable :: amat,dmat,ham,hamd,dinv,dmatc,vr,vl,xamp,dmatc_orig
...
double precision, dimension (:,:), allocatable :: vt,vrt



nlam=0
namex='1phon_hole/1f_x.dat'
namec='1phon_hole/1f_c.dat'
names='1phon_hole/1f_states.dat'
...
! call dmat_sp_subset(dim_base,dim_baser,phonbs,dmat,s,vt)
...
iout=1
     if (iout.eq.1) then
      ...
      write(99,*)
      do i=1,dim_baser
        write(99,'(1000f11.6)')(ham(i,j),j=1,dim_baser)
      enddo
     endif

do i=1,dim_baser
 do j=i,dim_baser
   if (dabs(ham(i,j)-ham(j,i)).gt.0.01d0) write(*,'(2i5,3f10.5)'),i,j,dabs(ham(i,j)-ham(j,i))
 enddo
enddo

! transformation of AD 
!  deallocate(amat)
!  allocate(amat(dim_baser,dim_baser))
!  amat=0.0d0

!  call dgemm('N','T',dim_baser,dim_baser,dim_baser,1.d0,ham,dim_baser,vt,dim_baser,0.d0,amat,dim_baser)
!  ham=0.d0
!  call dgemm('N','N',dim_baser,dim_baser,dim_baser,1.d0,vt,dim_baser,amat,dim_baser,0.d0,ham,dim_baser)

!iout=1
!     if (iout.eq.1) then
!      write(99,*)
!      write(99,*)'******** transfromed matrix  AD *************'
!      write(99,*)
!      do i=1,dim_baser
!        write(99,'(1000f11.6)')(ham(i,j),j=1,dim_baser)
!      enddo
...
! elimination of independent states from transformation 
!call reduce_rows(nx,vt,dim_baser,dim_ind)
...
!    spurious subspace elimination

 call dmat_sp_subset(dim_base,dim_baser,dim_ind,phonbs,dmat,ind_red,nx,s,vt,ns)
...
! transformation of reduced AD
  ...
  call dgemm('N','T',dim_ind,dim_ind,dim_ind,1.d0,ham,dim_ind,vt,dim_ind,0.d0,amat,dim_ind)
  ham=0.d0
  call dgemm('N','N',dim_ind,dim_ind,dim_ind,1.d0,vt,dim_ind,amat,dim_ind,0.d0,ham,dim_ind)
...
iout=1
     if (iout.eq.1) then
      ...
      write(99,*)
      do i=1,dim_ind
        write(99,'(1000f11.6)')(ham(i,j),j=1,dim_ind)
      enddo
     endif
...
allocate(dmatc_orig(dim_ind,dim_ind))
dmatc_orig=dmatc
...
!  allocate(amat(dim_ind,dim_ind))
  ...
  call dgemm('N','T',dim_ind,dim_ind,dim_ind,1.d0,dmatc,dim_ind,vt,dim_ind,0.d0,amat,dim_ind)
  dmatc=0.d0
  call dgemm('N','N',dim_ind,dim_ind,dim_ind,1.d0,vt,dim_ind,amat,dim_ind,0.d0,dmatc,dim_ind)

iout=1
     if (iout.eq.1) then
      write(99,*)
      write(99,*)'******** transfromed matrix  D *************'
      write(99,*)
      do i=1,dim_ind
        write(99,'(1000f11.6)')(dmatc(i,j),j=1,dim_ind)
      enddo
     endif
...
write(99,*)' '
write(99,'(1000f15.10)')(wr(j),j=1,dim_ind)
...
!   transformation of C to previous basis 
...
enddo
  ...
  call dgemm('T','N',dim_ind,dim_ind,dim_ind,1.d0,vt,dim_ind,vr,dim_ind,0.d0,amat,dim_ind)
vr=amat
...
!allocate(vrt(dim_baser,dim_ind))
!call dgemm('T','N',dim_baser,dim_ind,dim_ind,1.d0,vt,dim_ind,vr,dim_ind,0.d0,vrt,dim_baser)

!write(99,*)' '
!write(99,*)' C matrix transformed '
!do i=1,dim_baser
! write(99,'(1000f15.10)')(vrt(i,j),j=1,dim_ind)
!enddo
...
open(1,file=' phonon_base_ind.dat',status='unknown',form='formatted')
   write(1,*)'i   2*Tz    i_sp      en_sp      lam      en_phon'
    do i=1,dim_ind
     write(1,'(4i5,f10.5,i5,f10.5)')i,ind_red_ind(i),phonbs(ind_red_ind(i))%tz,phonbs(ind_red_ind(i))%isp,lev(phonbs(ind_red_ind(i))%isp)%en,phonbs(ind_red_ind(i))%ila,phon(phonbs(ind_red_ind(i))%ila)%enf
    enddo
